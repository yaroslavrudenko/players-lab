...
docker-build:env ## - Build Docker image for  application
	...
	@docker buildx build --build-arg "PLAYERS_CONFIG_SOURCE=$(PLAYERS_CONFIG_SOURCE)" --build-arg "LOG_SOURCE=$(LOG_SOURCE)" --build-arg "CONFIG_SOURCE=$(CONFIG_SOURCE)" --build-arg "APP_VERSION=$(APP_VERSION)" --build-arg "APP_BUILD=$(APP_BUILD)" --build-arg "APP_FULL_NAME=$(APP_FULL_NAME)" --build-arg "AWS_REGION=$(AWS_REGION)" --build-arg "AWS_ACCOUNT_NUMBER=$(AWS_ACCOUNT_NUMBER)"  -t $(DOCKER_IMAGE):$(APP_VERSION) -f DockerfileApp . --progress=plain --no-cache --platform linux/amd64,linux/arm64 --load
